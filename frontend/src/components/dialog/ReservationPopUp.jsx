...
const ReservationPopUp = ({open, onClose, reservation, openResult, handleOpenResult}) => {
    const user = getUser();
    const [doctor, setDoctor] = useState([]);
    const [selectedDoctor, setSelectedDoctor] = useState(null);
    const [position, setPosition] = useState("50%");
    const handleUpdate = () => {
        appointmentApi.assignDoctor(reservation?.id, selectedDoctor).then((res) => {
    ...
    useEffect(() => {
        if (reservation) {
            medicalServiceApi.getDoctor(reservation?.medical_service.id,reservation?.id).then((res) => {
                setDoctor(res.data);
            ...
            <DialogContent>
                <div className="flex border-[1.5px] border-gray-300 rounded-lg items-center gap-5 p-5">
                    <Avatar sx={{ width: 56, height: 56, bgcolor: getRandomColor() }}>
                        {reservation?.health_profile.name.split(' ').map(word => word[0]).join('')}
                    </Avatar>
                    <div>
                        <p className="text-gray-500">Patient Name</p>
                        <strong className="text-xl">{reservation?.health_profile.name}</strong>
                    </div>
                ...
                <div className="flex items-start justify-between mt-5">
                    ...
                    <div className="flex items-start gap-2">
                        ...
                        <div>
                            <p className="text-xs uppercase text-gray-400 mb-1">Ngày khám</p>
                            <p className="text-sm">{new Date(reservation?.date).toLocaleDateString("vi-VN")}</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-2">
                        <div className="flex items-center p-2 rounded-md bg-slate-100">
                        ...
                        <div>
                            ...
                            <p className="text-sm">{reservation?.start_time} - {reservation?.end_time}</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-5 w-full py-2">
                       <div>
                            ...
                            <p className="text-sm font-semibold text-gray-800">{reservation?.health_profile.name}</p>
                       </div>
                        <div>
                             <p className="text-xs uppercase text-gray-500">Ngày sinh</p>
                             <p className="text-sm font-semibold text-gray-800">{reservation?.health_profile.date_of_birth}</p>
                        </div>
                        <div>
                            <p className="text-xs uppercase text-gray-500">Giới tính</p>
                            <p className="text-sm font-semibold text-gray-800">{reservation?.health_profile.gender}</p>
                        </div>
                        <div>
                            <p className="text-xs uppercase text-gray-500">Số điện thoại</p>
                            <p className="text-sm font-semibold text-gray-800">0123 345 539</p>
                        </div>
                        <div>
                            <p className="text-xs uppercase text-gray-500">Email</p>
                            <p className="text-sm font-semibold text-gray-800">example@example.com</p>
                        </div>
                    </div>
                </div>
                <Divider />
                    ...
                    </Select>
                    {
                        selectedDoctor && (
                            <div className="flex items-center gap-2 mt-4 p-2">
                                <Avatar src={doctor.find(doc => doc.id === selectedDoctor)?.avatar} sx={{ width: 56, height: 56 }} />
                                <div>
                                    <p className="text-sm font-semibold">{doctor.find(doc => doc.id === selectedDoctor)?.name}</p>
                                    <p className="text-sm font-semibold">{doctor.find(doc => doc.id === selectedDoctor)?.position}</p>
                                    <p className="text-xs text-gray-500">Chuyên khoa: {doctor.find(doc => doc.id === selectedDoctor)?.specialization}</p>
                                </div>
                            </div>
                        )
                    }
                ...
                <DialogActions sx={{ padding: "16px" }}>
                    {
                        user.role !== "3" ?
                        <Button variant="contained" color="primary" onClick={handleUpdate} disabled={selectedDoctor == reservation?.doctor?.id}>
                            Cập nhật
                        </Button>
                        :
